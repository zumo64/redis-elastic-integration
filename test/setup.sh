...
for arg in "$@"; do
  ...
  if [ "$arg" == "--create-db" ]; then
    ...
    # create 2 DB
	curl -X POST -H 'cache-control: no-cache' -H 'Content-type: application/json' -u redis@redis.com:redis --retry 20  --retry-connrefused -d '{ "bdb": {"uid":1,"name": "zumo-test-db1","type": "redis","memory_size": 10000,"shards_count":1, "authentication_redis_pass":"redis", "port" : 10670 }}' -k https://localhost:9443/v2/bdbs
	sleep 5
	rladmin tune db db:1 db_conns_auditing enabled
	sleep 5
	curl -X POST -H 'cache-control: no-cache' -H 'Content-type: application/json' -u redis@redis.com:redis --retry 20 --retry-connrefused -d '{ "bdb": {"uid":2,"name": "zumo-test-db2","type": "redis","memory_size": 10000,"shards_count":1, "authentication_redis_pass":"redis", "port" : 10671 }}' -k https://localhost:9443/v2/bdbs
	sleep 5 
  ...
  if [ "$arg" == "--run-connect" ]; then
    ...
    for value in $(seq 1 "$3") ; do
    	...
    	redis-cli -h localhost -p $dbport -a redis --latency -i $delay > /dev/null
    done
  fi
